...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.load_button = ctk.CTkButton(file_frame, text="Excelファイルを選択", command=self.load_file)
        ...
        self.column_selector = ctk.CTkComboBox(column_frame, state="disabled", values=[])
        ...
        self.run_button = ctk.CTkButton(run_frame, text="分析実行", command=self.run_analysis_wrapper, state="disabled")
        ...
        self.save_excel_button = ctk.CTkButton(save_frame, text="分析結果をExcelに保存", command=self.save_excel, state="disabled")
        self.save_excel_button.pack(side="left", padx=10, pady=10, expand=True)

        self.save_pdf_button = ctk.CTkButton(save_frame, text="サマリーPDFを保存", command=self.save_pdf, state="disabled")
        self.save_pdf_button.pack(side="left", padx=10, pady=10, expand=True)

        self.save_wordcloud_button = ctk.CTkButton(save_frame, text="ワードクラウドを保存", command=self.save_wordcloud, state="disabled")
        self.save_wordcloud_button.pack(side="left", padx=10, pady=10, expand=True)

        # --- APIキーチェック ---
        if not settings.OPENAI_API_KEY:
            messagebox.showerror("設定エラー", "OPENAI_API_KEYが環境変数に設定されていません。")
            self.destroy()
    ...
    def load_file(self):
        ...
        try:
            ...
            self.column_selector.configure(values=self.df.columns.tolist(), state="normal")
    ...
    def run_analysis_wrapper(self):
        ...
        async def run():
            try:
                self.df_analyzed = await analyze_dataframe(self.df, column, progress_callback=self.update_progress)
                self.summary_data, self.wordcloud_words = summarize_results(self.df_analyzed)
                ...
                messagebox.showerror("分析エラー", f"分析中にエラーが発生しました:\n{e}")
            finally:
                self.run_button.configure(state="normal")
