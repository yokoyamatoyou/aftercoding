...
`NotoSansJP-Regular.otf` を配置してください。フォントは
[Google Fonts](https://fonts.google.com/noto/specimen/Noto+Sans+JP)
からダウンロードできます。ファイルが見つからない場合は、
Windowsの標準フォント（メイリオなど）が自動的に使用されますが、
...
- **グラフやPDFの日本語が四角（豆腐）になる:**
  - `fonts/NotoSansJP-Regular.otf` を配置していない場合、環境によって日本語が正しく表示されないことがあります。フォントを追加して再実行してください。
